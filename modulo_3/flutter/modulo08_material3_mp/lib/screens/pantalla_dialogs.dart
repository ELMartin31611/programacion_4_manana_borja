...
class PantallaDialogs extends StatelessWidget {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text(esError
            ? 'Error: no se pudo enviar la solicitud de adopción'
  ...
  Future<void> _mostrarConfirmacion(BuildContext context) async {
    final confirmar = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        title:   const Text('Cancelar adopción'),
        content: const Text(
          '¿Estás seguro de que deseas cancelar la adopción de Luna?\n'
    ...
    if (confirmar == true) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Adopción cancelada correctamente')),
      );
    }
  }
  ...
  Future<void> _mostrarFormulario(BuildContext context) async {
    ...
    final ctrlRaza   = TextEditingController();
        ...
        content: Form(
          ...
          child: Column(
            ...
            children: [
              ...
              TextFormField(
                controller: ctrlRaza,
                decoration: const InputDecoration(labelText: 'Código de chip'),
                validator:  (v) {
    ...
    if (ctrlNombre.text.isNotEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Mascota "${ctrlNombre.text}" registrada')),
      );
    }
  }
      ...
      body: ListView(
        ...
        children: [
          ...
          OutlinedButton.icon(
            ...
            label: const Text('Cancelar adopción (confirmación)'),
          ),
          const SizedBox(height: 8),
          FilledButton.tonal(
            onPressed: () => _mostrarFormulario(context),
            child: const Text('Registrar mascota (formulario)'),
          ),
        ],
      ),
    );
